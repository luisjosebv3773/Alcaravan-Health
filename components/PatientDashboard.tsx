...
import { Radar, RadarChart, PolarGrid, PolarAngleAxis, PolarRadiusAxis, ResponsiveContainer, BarChart, Bar, Cell } from 'recharts';

const data = [
  { subject: 'Muscle', A: 120, B: 110, fullMark: 150 },
  { subject: 'Fat %', A: 98, B: 130, fullMark: 150 },
  { subject: 'Water', A: 86, B: 130, fullMark: 150 },
  { subject: 'Bone', A: 99, B: 100, fullMark: 150 },
  { subject: 'Protein', A: 85, B: 90, fullMark: 150 },
  { subject: 'Metabolism', A: 65, B: 85, fullMark: 150 },
];

const trendData = [
  { name: 'Month 1', value: 78.5 },
  { name: 'Month 2', value: 78.2 },
  { name: 'Month 3', value: 77.9 },
  { name: 'Month 4', value: 78.3 },
  { name: 'Month 5', value: 78.1 },
  { name: 'Month 6', value: 78.0 },
];
    ...
    <div className="max-w-[1400px] mx-auto px-6 py-8">
      <section className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="max-w-2xl">
          <h2 className="text-3xl md:text-4xl font-black tracking-tight mb-2">Good morning, Mateo</h2>
          <p className="text-text-sub dark:text-gray-400 text-lg">Your latest measurements show promising trends. Let's keep your stats <span className="text-primary font-bold">green</span> today.</p>
        </div>
        <div className="flex gap-3">
          <button className="flex items-center gap-2 px-5 py-2.5 bg-white dark:bg-card-dark border border-gray-200 dark:border-gray-700 rounded-lg text-sm font-bold shadow-sm hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
            <span className="material-symbols-outlined !text-[20px]">download</span> Full Report
          </button>
          <button className="flex items-center gap-2 px-5 py-2.5 bg-primary hover:bg-primary-dark text-black rounded-lg text-sm font-bold shadow-md shadow-primary/20 transition-colors">
            <span className="material-symbols-outlined !text-[20px]">add</span> Log Activity
          </button>
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-8 flex flex-col gap-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <MetricCard icon="monitor_weight" label="IMC (Body Mass Index)" value="24.5" status="Normal" color="status-green" subtext="Your weight is optimal for your height." />
            <MetricCard icon="straighten" label="ICC (Waist-Hip Ratio)" value="0.86" status="Moderate" color="status-amber" subtext="Slightly elevated risk. Monitor diet." />
            <MetricCard icon="cardiology" label="Visceral Fat Level" value="9" status="Healthy" color="status-green" subtext="Internal fat levels are within safe range." />
          </div>
          ...
          <div className="bg-card-light dark:bg-card-dark rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 p-6 flex flex-col min-h-[400px]">
            <div className="flex flex-wrap justify-between items-center mb-6 gap-4">
              <div>
                <h3 className="text-xl font-bold">Body Composition Analysis</h3>
                <p className="text-sm text-text-sub dark:text-gray-400">Radar comparison: You vs. Ideal Targets</p>
              </div>
              ...
              <div className="w-full md:w-1/2 h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
                  <RadarChart cx="50%" cy="50%" outerRadius="80%" data={data}>
                    ...
                    <Radar name="You" dataKey="A" stroke="#13ec5b" fill="#13ec5b" fillOpacity={0.6} />
                    <Radar name="Target" dataKey="B" stroke="#9ca3af" fill="#9ca3af" fillOpacity={0.2} strokeDasharray="4 4" />
                  </RadarChart>
                </ResponsiveContainer>
              </div>
              <div className="flex-1 grid grid-cols-2 gap-x-8 gap-y-6 w-full">
                <SubMetric label="Muscle Mass" value="38.2" unit="kg" progress={85} trend="+2.1kg" color="primary" />
                <SubMetric label="Body Fat" value="18.5" unit="%" progress={45} trend="-0.5%" color="status-amber" />
                <SubMetric label="Body Water" value="58.4" unit="%" progress={70} trend="Optimal" color="primary" />
                <SubMetric label="Metabolism" value="1740" unit="kcal" progress={90} trend="High" color="primary" />
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="mt-8">
        <h3 className="text-xl font-bold mb-6">Metric Trends (Last 6 Months)</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <TrendCard label="Weight" value="78kg" trend="-2%" color="primary" />
          <TrendCard label="Muscle Mass" value="38.2kg" trend="+1.5%" color="primary" />
          <TrendCard label="Sleep Avg" value="7h 12m" trend="0%" color="status-amber" />
          <TrendCard label="Visceral Fat" value="Level 9" trend="Good" color="primary" />
        </div>
      </section>
    </div>
  );
  ...
  return (
    <div className="bg-card-light dark:bg-card-dark rounded-xl p-6 shadow-sm border border-gray-100 dark:border-gray-800 hover:shadow-md transition-shadow group">
      <div className="flex justify-between items-start mb-4">
        ...
        <span className={`inline-flex rounded-full h-3 w-3 ${status === 'Normal' || status === 'Healthy' ? 'bg-primary' : 'bg-status-amber'}`}></span>
      </div>
  ...
  return (
    <div className="bg-card-light dark:bg-card-dark rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 p-6">
      ...
      <div className="relative pl-6 pb-6 border-l-2 border-primary/30 last:pb-0">
        ...
        <p className="text-sm font-bold text-text-sub mb-1">Tomorrow, 10:00 AM</p>
        <h4 className="font-bold text-lg mb-1">Nutritionist Consultation</h4>
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-sm font-medium">Dr. Sarah Jimenez</span>
        </div>
        <div className="flex gap-2">
          <button className="px-3 py-1.5 text-xs font-bold bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded transition-colors">Reschedule</button>
          <button className="px-3 py-1.5 text-xs font-bold text-primary bg-primary/10 hover:bg-primary/20 rounded transition-colors">Details</button>
        </div>
      </div>
      <div className="mt-6 pt-6 border-t border-gray-100 dark:border-gray-800">
        <button className="w-full py-3 bg-primary text-black font-bold rounded-lg hover:bg-primary-dark transition-colors flex items-center justify-center gap-2 shadow-lg shadow-primary/20">
          <span className="material-symbols-outlined">calendar_add_on</span> Book New Appointment
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div className="bg-background-light dark:bg-background-dark border border-dashed border-gray-300 dark:border-gray-700 rounded-xl p-6 flex flex-col gap-4">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h3 className="text-lg font-bold">Daily Wellness Goals</h3>
      </div>
      {[
        { title: "Drink 2.5L Water", sub: "Current: 1.2L", done: false },
        { title: "15 min stretching", sub: "For back health", done: false },
        { title: "Take Vitamins", sub: "Completed at 8:30 AM", done: true },
      ].map((g, i) => (
...
function EducationTip() {
  return (
    <div className="bg-gradient-to-br from-[#102216] to-[#1a3826] rounded-xl p-6 text-white relative overflow-hidden group">
      ...
      <span className="inline-block px-2 py-1 bg-white/20 rounded text-[10px] font-bold uppercase tracking-wider mb-3 backdrop-blur-sm">Tip of the day</span>
      <h4 className="font-bold text-lg mb-2 relative z-10">Improving Waist-Hip Ratio</h4>
      <p className="text-sm text-gray-300 mb-4 relative z-10">Adding 20 mins of cardio can significantly improve your ICC metrics over 4 weeks.</p>
