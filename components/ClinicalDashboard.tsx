  ...
  return (
    <div className="max-w-[1600px] mx-auto p-4 md:p-6 lg:p-8 flex flex-col gap-6 lg:gap-8 bg-background-light dark:bg-background-dark">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <h2 className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white tracking-tight">Good Morning, Dr. Alcaraván</h2>
          <p className="text-slate-500 dark:text-slate-400 mt-1 flex items-center gap-2">
            <span className="material-symbols-outlined text-sm">calendar_today</span>
            Monday, Oct 24 • <span className="text-slate-900 dark:text-white font-semibold">8 appointments</span> scheduled.
          </p>
        </div>
        <div className="flex gap-3 w-full md:w-auto">
          <button className="flex-1 md:flex-none bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 font-bold py-2.5 px-4 rounded-lg shadow-sm hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center justify-center gap-2 transition-all">
            <span className="material-symbols-outlined">print</span> Daily Report
          </button>
          <button className="flex-1 md:flex-none bg-primary text-slate-900 hover:bg-[#0fdc52] font-bold py-2.5 px-6 rounded-lg shadow-lg shadow-green-500/20 flex items-center justify-center gap-2 transition-all transform hover:-translate-y-0.5">
            <span className="material-symbols-outlined">add_circle</span> Consultation
          </button>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6">
        <StatCard icon="calendar_month" color="blue" label="Total Appointments" value="8" trend="+20%" />
        <StatCard icon="hourglass_top" color="orange" label="Pending Requests" value="3" />
        <StatCard icon="monitor_heart" color="red" label="Urgent Cases" value="1" alert />
      </div>
        ...
        <div className="xl:col-span-8 bg-card-light dark:bg-card-dark rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col overflow-hidden">
          <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row justify-between items-center gap-4">
            <h3 className="font-bold text-lg flex items-center gap-2">
              <span className="material-symbols-outlined text-slate-400">calendar_view_day</span> Visual Scheduler
            </h3>
            <div className="bg-slate-100 dark:bg-slate-800 p-1 rounded-lg flex text-sm font-medium">
              <button className="px-4 py-1.5 bg-white dark:bg-card-dark shadow-sm rounded-md text-slate-900 dark:text-white border border-slate-200 dark:border-slate-700">Appointments</button>
              <button className="px-4 py-1.5 text-slate-500 hover:text-slate-900 dark:hover:text-white">Availability</button>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-auto relative p-0 bg-slate-50/50 dark:bg-card-dark/50 no-scrollbar">
            <div className="grid grid-cols-[60px_1fr] h-full min-w-[600px]">
              ...
              <div className="relative bg-white dark:bg-card-dark">
                ...
                <ScheduleItem top={4} height={72} color="primary" name="Maria Gonzalez" type="General Checkup" status="Confirmed" />
                <ScheduleItem top={161} height={78} color="blue" name="Carlos Rodriguez" type="Follow-up: Hypertension" status="In Progress" />
                <div className="absolute top-[82px] left-2 right-2 h-[76px] border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-lg flex items-center justify-center cursor-pointer hover:border-primary hover:bg-primary/5 transition-all group">
                   <span className="text-sm text-slate-400 group-hover:text-primary font-medium flex items-center gap-1"><span className="material-symbols-outlined text-lg bg-slate-100 dark:bg-slate-800 rounded-full p-0.5">add</span> Available Slot</span>
                </div>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="bg-card-light dark:bg-card-dark p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-between h-36 relative overflow-hidden group hover:shadow-md transition-shadow">
      ...
      <div className="flex justify-between items-start z-10">
        ...
        {alert && <span className="text-red-600 dark:text-red-400 text-xs font-bold bg-red-100 dark:bg-red-900/30 px-2 py-1 rounded-full animate-pulse">Alert</span>}
      </div>
      <div>
...
function NextPatientCard() {
  return (
    <div className="bg-card-light dark:bg-card-dark rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-5 relative overflow-hidden">
      ...
      <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">Up Next (11:00 AM)</h4>
      ...
      <div className="grid grid-cols-2 gap-2 mb-5">
        <div className="bg-slate-50 dark:bg-slate-800/50 p-2 rounded-lg border border-slate-100 dark:border-slate-800">
          ...
          <p className="text-sm font-semibold text-orange-500 flex items-center gap-1"><span className="material-symbols-outlined text-sm">warning</span> Moderate</p>
        </div>
        <div className="bg-slate-50 dark:bg-slate-800/50 p-2 rounded-lg border border-slate-100 dark:border-slate-800">
      ...
      <div className="flex gap-2">
        <span className="text-xs bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 px-2 py-1 rounded font-medium border border-red-100 dark:border-red-900/30">Diabetic Type 2</span>
        <span className="text-xs bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400 px-2 py-1 rounded font-medium border border-orange-100 dark:border-orange-900/30">High BP</span>
      </div>
      <div className="mt-5 pt-4 border-t border-slate-100 dark:border-slate-800 flex gap-3">
        <button className="flex-1 bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold py-2.5 rounded-lg text-sm hover:opacity-90 transition-opacity">Start Consultation</button>
  ...
  return (
    <div className="bg-card-light dark:bg-card-dark rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex-1 flex flex-col min-h-[300px]">
      <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/20">
        <h3 className="font-bold text-sm flex items-center gap-2"><span className="size-2 bg-orange-500 rounded-full animate-pulse"></span> Requests (3)</h3>
      ...
      <div className="p-3 overflow-y-auto flex flex-col gap-3">
        {[
          { name: "Sarah Lee", time: "Tomorrow, 9:00 AM", note: "Feeling persistent headaches...", img: "p1" },
